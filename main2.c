...
#define KMAX	125
static double bufferVout[IMAX];			// voltage out buffer
static double *bpVout = bufferVout;		// voltage out buffer pointer
static double bufferVtemp[IMAX];		// voltage before filter buffer
static double *bpVtemp = bufferVtemp;	// voltage before filter buffer pointer
static double bufferXcart[IMAX];		// cart position buffer
static double *bpXcart = bufferXcart;	// cart position buffer pointer
static double bufferTheta[IMAX];		// angle buffer
static double *bpTheta = bufferTheta;	// angle buffer pointer
static double bufferXball[IMAX];		// ball position buffer
static double *bpXball = bufferXball;	// ball position buffer pointer
static double bufferTime[IMAX];			// time buffer
static double *bpTime = bufferTime;		// time buffer pointer
...
void *Timer_Irq_Thread(void* resource) {
	...
	MATFILE *mf;
	int err;
	double Xc;			// Cart position
	double Xb;			// Ball (payload) position
	double Vc;			// Cart velocity
	double Th;			// Angle (Degrees)
	double Vout;		// Output voltage
	double Vtemp;		// Voltage before filtering
	double Xcomp = 0;	// Position compensation for step response
	double Xcartprev = 0;
//	double Vreq = 0;
...
//	double Vdrop = 0;	// Oldest value of voltage array, being replaced
	...
	double targetD;
	double t = 0;
//	double t_inc = (double) timeoutValue;
	double currentP = 0;
...
//				Vout = (V1 + V2)/2.5;
			...
			*VDA = round(1000*Vout)/1000.0;
			...
			if (bpVtemp < bufferVtemp + IMAX) *bpVtemp++ = Vtemp;	// Store voltage data in array

			if (bpVout < bufferVout + IMAX) *bpVout++ = Vout;	// Store voltage data in array
...
//			// ************************* FRICTION COMPENSATION ****************************
			...
			if (bpXcart < bufferXcart + IMAX) *bpXcart++ = Xc;	// Store position data in array
			if (bpXball < bufferXball + IMAX) *bpXball++ = Xb;	// Store angle data in array
			if (bpTheta < bufferTheta + IMAX) *bpTheta++ = Th;	// Store angle data in array
			if (bpTime  < bufferTime  + IMAX) *bpTime++  = t;	// Store angle data in array
...
//				printf("/f%f", targetD);
    ...
    matfile_addmatrix(mf, "Vout",  bufferVout,  IMAX, 1, 0);
    matfile_addmatrix(mf, "Vtemp", bufferVtemp, IMAX, 1, 0);
    matfile_addmatrix(mf, "Xcart", bufferXcart, IMAX, 1, 0);
    matfile_addmatrix(mf, "Xball", bufferXball, IMAX, 1, 0);
    matfile_addmatrix(mf, "Theta", bufferTheta, IMAX, 1, 0);
    matfile_addmatrix(mf, "t",     bufferTime,  IMAX, 1, 0);
